...
| ~--with-native-comp~      | Enable native-comp support (only for emacs-head@28)                          |
| ~--with-native-full-aot~  | Enable Ahead-of-Time native compilation support (only for emacs-head@28)     |
...
enable native full AoT, please use ~--with-native-comp~ option
