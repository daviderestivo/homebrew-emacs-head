...
| ~--with-nativecomp~       | Enable native-comp support (only for emacs-head@28)                          |
...
brew install daviderestivo/emacs-head/gcc --build-from-source --force
brew install emacs-head@28 --with-cocoa --with-nativecomp
#+end_src
